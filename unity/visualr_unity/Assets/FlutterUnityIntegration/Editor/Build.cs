...
public class Build : EditorWindow
{
    ...
    static readonly string apkPath = Path.Combine(
        ProjectPath,
        "Builds/" + Application.productName + ".apk"
    );

    static readonly string androidExportPath = Path.GetFullPath(
        Path.Combine(ProjectPath, "../../android/unityLibrary")
    );
    static readonly string iosExportPath = Path.GetFullPath(
        Path.Combine(ProjectPath, "../../ios/UnityLibrary")
    );
    static readonly string iosExportPluginPath = Path.GetFullPath(
    ...
    private static void BuildIOS(String path)
    {
        ...
        var options = BuildOptions.AllowDebugging;
        var report = BuildPipeline.BuildPlayer(GetEnabledScenes(), path, BuildTarget.iOS, options);
    ...
    static void SetupAndroidProject()
    {
        ...
        if (!Regex.IsMatch(proj_build_script, @"flatDir[^/]*[^}]*}"))
        {
            ...
            proj_build_script = regex.Replace(
                proj_build_script,
                @"
allprojects {
    repositories {
...
";
        ...
        else
        {
            if (!app_build_script.Contains(@"implementation project(':unityLibrary')"))
            {
                ...
                app_build_script = regex.Replace(
                    app_build_script,
                    @"
dependencies {
    implementation project(':unityLibrary')
...
";
    ...
    static void SetupIOSProjectForPlugin()
    {
        ...
        if (
            !Regex.IsMatch(pubsec_text, @"\w\.xcconfig(?:[^}]*})+")
            && !Regex.IsMatch(pubsec_text, @"tar -xvjf UnityFramework.tar.bz2")
        )
        {
            Regex regex = new Regex(@"\w\.xcconfig(?:[^}]*})+", RegexOptions.Multiline);
            pubsec_text = regex.Replace(
                pubsec_text,
                @"
	spec.xcconfig = {
    ...
    static async void BuildUnityFrameworkArchive()
    {
        ...
        string IOS_ARCHIVE_FRAMEWORK_PATH =
            $"{BUILD_PATH}/{IOS_ARCHIVE_DIR}/Products/Library/Frameworks/{FRAMEWORK_NAME_WITH_EXT}";
        ...
        try
        {
            ...
            await $"archive -workspace {XCPROJECT_NAME} -scheme {SCHEME_NAME} -sdk iphoneos -archivePath {BUILD_PATH}/Release-iphoneos.xcarchive ENABLE_BITCODE=NO |xcpretty".Bash(
                "xcodebuild"
            );

            Debug.Log("### Copying framework files");
            await $" -RL {IOS_ARCHIVE_FRAMEWORK_PATH} {IOS_RUNNER_PATH}/{FRAMEWORK_NAME_WITH_EXT}".Bash(
                "cp"
            );
            await $" -RL {IOS_ARCHIVE_FRAMEWORK_PATH}/{DYSM_NAME_WITH_EXT} {IOS_RUNNER_PATH}/{DYSM_NAME_WITH_EXT}".Bash(
                "cp"
            );
